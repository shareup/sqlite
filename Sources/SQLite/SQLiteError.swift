...
public extension SQLiteError {
    ...
    static let SQLITE_READONLY =
        SQLiteError(rawValue: 8) // Attempt to write a readonly database
    static let SQLITE_INTERRUPT =
        SQLiteError(rawValue: 9) // Operation terminated by sqlite3_interrupt()
    static let SQLITE_IOERR = SQLiteError(rawValue: 10) // Some kind of disk I/O error occurred
    static let SQLITE_CORRUPT =
        SQLiteError(rawValue: 11) // The database disk image is malformed
    static let SQLITE_NOTFOUND =
        SQLiteError(rawValue: 12) // Unknown opcode in sqlite3_file_control()
    static let SQLITE_FULL =
        SQLiteError(rawValue: 13) // Insertion failed because database is full
    ...
    static let SQLITE_CONSTRAINT =
        SQLiteError(rawValue: 19) // Abort due to constraint violation
    ...
    static let SQLITE_NOLFS =
        SQLiteError(rawValue: 22) // Uses OS features not supported on host
    ...
    static let SQLITE_RANGE =
        SQLiteError(rawValue: 25) // 2nd parameter to sqlite3_bind out of range
    static let SQLITE_NOTADB =
        SQLiteError(rawValue: 26) // File opened that is not a database file
    ...
    static let SQLITE_ERROR_MISSING_COLLSEQ = SQLiteError(
        rawValue: SQLITE_ERROR
            .rawValue | (1 << 8)
    )
    ...
    static let SQLITE_IOERR_CHECKRESERVEDLOCK = SQLiteError(
        rawValue: SQLITE_IOERR
            .rawValue | (14 << 8)
    )
    ...
    static let SQLITE_IOERR_DELETE_NOENT = SQLiteError(
        rawValue: SQLITE_IOERR
            .rawValue | (23 << 8)
    )
    static let SQLITE_IOERR_MMAP = SQLiteError(rawValue: SQLITE_IOERR.rawValue | (24 << 8))
    static let SQLITE_IOERR_GETTEMPPATH = SQLiteError(
        rawValue: SQLITE_IOERR
            .rawValue | (25 << 8)
    )
    ...
    static let SQLITE_IOERR_BEGIN_ATOMIC = SQLiteError(
        rawValue: SQLITE_IOERR
            .rawValue | (29 << 8)
    )
    static let SQLITE_IOERR_COMMIT_ATOMIC = SQLiteError(
        rawValue: SQLITE_IOERR
            .rawValue | (30 << 8)
    )
    static let SQLITE_IOERR_ROLLBACK_ATOMIC = SQLiteError(
        rawValue: SQLITE_IOERR
            .rawValue | (31 << 8)
    )
    ...
    static let SQLITE_LOCKED_SHAREDCACHE = SQLiteError(
        rawValue: SQLITE_LOCKED
            .rawValue | (1 << 8)
    )
    ...
    static let SQLITE_CANTOPEN_NOTEMPDIR = SQLiteError(
        rawValue: SQLITE_CANTOPEN
            .rawValue | (1 << 8)
    )
    static let SQLITE_CANTOPEN_ISDIR = SQLiteError(
        rawValue: SQLITE_CANTOPEN
            .rawValue | (2 << 8)
    )
    static let SQLITE_CANTOPEN_FULLPATH = SQLiteError(
        rawValue: SQLITE_CANTOPEN
            .rawValue | (3 << 8)
    )
    static let SQLITE_CANTOPEN_CONVPATH = SQLiteError(
        rawValue: SQLITE_CANTOPEN
            .rawValue | (4 << 8)
    )
    static let SQLITE_CANTOPEN_DIRTYWAL = SQLiteError(
        rawValue: SQLITE_CANTOPEN
            .rawValue | (5 << 8)
    ) /* Not Used */
    static let SQLITE_CANTOPEN_SYMLINK = SQLiteError(
        rawValue: SQLITE_CANTOPEN
            .rawValue | (6 << 8)
    )
    static let SQLITE_CORRUPT_VTAB = SQLiteError(rawValue: SQLITE_CORRUPT.rawValue | (1 << 8))
    static let SQLITE_CORRUPT_SEQUENCE = SQLiteError(
        rawValue: SQLITE_CORRUPT
            .rawValue | (2 << 8)
    )
    static let SQLITE_CORRUPT_INDEX = SQLiteError(rawValue: SQLITE_CORRUPT.rawValue | (3 << 8))
    static let SQLITE_READONLY_RECOVERY = SQLiteError(
        rawValue: SQLITE_READONLY
            .rawValue | (1 << 8)
    )
    static let SQLITE_READONLY_CANTLOCK = SQLiteError(
        rawValue: SQLITE_READONLY
            .rawValue | (2 << 8)
    )
    static let SQLITE_READONLY_ROLLBACK = SQLiteError(
        rawValue: SQLITE_READONLY
            .rawValue | (3 << 8)
    )
    static let SQLITE_READONLY_DBMOVED = SQLiteError(
        rawValue: SQLITE_READONLY
            .rawValue | (4 << 8)
    )
    static let SQLITE_READONLY_CANTINIT = SQLiteError(
        rawValue: SQLITE_READONLY
            .rawValue | (5 << 8)
    )
    static let SQLITE_READONLY_DIRECTORY = SQLiteError(
        rawValue: SQLITE_READONLY
            .rawValue | (6 << 8)
    )
    static let SQLITE_ABORT_ROLLBACK = SQLiteError(rawValue: SQLITE_ABORT.rawValue | (2 << 8))
    static let SQLITE_CONSTRAINT_CHECK = SQLiteError(
        rawValue: SQLITE_CONSTRAINT
            .rawValue | (1 << 8)
    )
    static let SQLITE_CONSTRAINT_COMMITHOOK = SQLiteError(
        rawValue: SQLITE_CONSTRAINT
            .rawValue | (2 << 8)
    )
    static let SQLITE_CONSTRAINT_FOREIGNKEY = SQLiteError(
        rawValue: SQLITE_CONSTRAINT
            .rawValue | (3 << 8)
    )
    static let SQLITE_CONSTRAINT_FUNCTION = SQLiteError(
        rawValue: SQLITE_CONSTRAINT
            .rawValue | (4 << 8)
    )
    static let SQLITE_CONSTRAINT_NOTNULL = SQLiteError(
        rawValue: SQLITE_CONSTRAINT
            .rawValue | (5 << 8)
    )
    static let SQLITE_CONSTRAINT_PRIMARYKEY = SQLiteError(
        rawValue: SQLITE_CONSTRAINT
            .rawValue | (6 << 8)
    )
    static let SQLITE_CONSTRAINT_TRIGGER = SQLiteError(
        rawValue: SQLITE_CONSTRAINT
            .rawValue | (7 << 8)
    )
    static let SQLITE_CONSTRAINT_UNIQUE = SQLiteError(
        rawValue: SQLITE_CONSTRAINT
            .rawValue | (8 << 8)
    )
    static let SQLITE_CONSTRAINT_VTAB = SQLiteError(
        rawValue: SQLITE_CONSTRAINT
            .rawValue | (9 << 8)
    )
    static let SQLITE_CONSTRAINT_ROWID = SQLiteError(
        rawValue: SQLITE_CONSTRAINT
            .rawValue | (10 << 8)
    )
    static let SQLITE_CONSTRAINT_PINNED = SQLiteError(
        rawValue: SQLITE_CONSTRAINT
            .rawValue | (11 << 8)
    )
    static let SQLITE_CONSTRAINT_DATATYPE = SQLiteError(
        rawValue: SQLITE_CONSTRAINT
            .rawValue | (12 << 8)
    )
    static let SQLITE_NOTICE_RECOVER_WAL = SQLiteError(
        rawValue: SQLITE_NOTICE
            .rawValue | (1 << 8)
    )
    static let SQLITE_NOTICE_RECOVER_ROLLBACK = SQLiteError(
        rawValue: SQLITE_NOTICE
            .rawValue | (2 << 8)
    )
    static let SQLITE_WARNING_AUTOINDEX = SQLiteError(
        rawValue: SQLITE_WARNING
            .rawValue | (1 << 8)
    )
...
public func ~= (lhs: SQLiteError, rhs: Error) -> Bool {
